...
class LiveShowMultiViewHolder(itemView: View) : androidx.recyclerview.widget.RecyclerView.ViewHolder(itemView) {

    var showItem: LiveShowModel? = null
    ...
    companion object {
        ...
        @JvmStatic
        fun create(parent: ViewGroup, @LayoutRes layout: Int): LiveShowMultiViewHolder {
            return LiveShowMultiViewHolder(parent.inflate(layout))
        }
    }

    init {
        itemView.clShows.layoutParams.height = (Utils.getScreenHeight() * 0.75f).toInt()
    }

    fun bindTo(item: LiveShowModel, listener: OnClickListener?) {
        with(itemView) {
            ...
            then R.drawable.masterbrain_default ?: R.drawable.user_image_default)
            showItem = item
            item.balance?.run {
    ...
    interface OnClickListener {
        ...
        fun onFollowClicked(item: LiveShowModel)
        fun onShowActionButtonClicked(item: LiveShowModel)
