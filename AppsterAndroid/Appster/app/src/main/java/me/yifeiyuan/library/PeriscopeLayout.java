...
public class PeriscopeLayout extends RelativeLayout {
    ...
    private void init() {
        ...
        Drawable lightpink = ContextCompat.getDrawable(getContext(), R.drawable.pl_lightpink);
        Drawable pink = ContextCompat.getDrawable(getContext(), R.drawable.pl_pink);
        Drawable purple = ContextCompat.getDrawable(getContext(), R.drawable.pl_purple);
        Drawable turquoise = ContextCompat.getDrawable(getContext(), R.drawable.pl_turquoise);

        drawables[0] = lightpink;
        drawables[1] = pink;
        drawables[2] = purple;
        drawables[3] = turquoise;
        //获取图的宽高 用于后面的计算
        //注意 我这里3张图片的大小都是一样的,所以我只取了一个
        dHeight = lightpink.getIntrinsicHeight();
        dWidth = lightpink.getIntrinsicWidth();
    ...
    private PointF getPointF(int scale) {
        ...
        if (mWidth > 100) {
            ...
            pointF.x = random.nextInt(mWidth + 1);
        }
        //再Y轴上 为了确保第二个点 在第一个点之上,我把Y分成了上下两半 这样动画效果好一些  也可以用其他方法
        if (mHeight > 100) {
            pointF.y = random.nextInt((mHeight - 100)) / scale;
        } else {
            pointF.y = random.nextInt(mHeight + 1) / scale;
        }
        return pointF;
    }

    private static class BezierListenr implements ValueAnimator.AnimatorUpdateListener {
