...
public class WelcomeViewPagerIndicator extends SimpleViewPagerIndicator implements OnWelcomeScreenPageChangeListener {
    ...
    private boolean shouldHideAtLastPage = true;
    ...
    public void onPageSelected(int position) {
        if (shouldHideAtLastPage) {
            final int visibility = position == getTotalPages() - 1 ? INVISIBLE : VISIBLE;
            if (getVisibility() != visibility) setVisibility(visibility);
        }
    }

    public void setShouldHideAtLastPage(boolean shouldHideAtLastPage) {
        this.shouldHideAtLastPage = shouldHideAtLastPage;
    }
}
