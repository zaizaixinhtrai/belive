...
public abstract class WelcomeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ((TextView) done.getView()).setBackground(ContextCompat.getDrawable(this, R.drawable.wel_button_done_background));
        if (configuration.getBottomLayoutResId() == WelcomeConfiguration.BottomLayout.BELIVE.resId) {
            if (done.getView() instanceof TextView && Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ((TextView) done.getView()).setLetterSpacing(0.27f);
            }
        }
